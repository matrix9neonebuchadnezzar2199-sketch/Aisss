...
export function AiSearchPage () {
  ...
  return (
    <section className="page ai-page">
      <h2>AI 検索</h2>
      <p className="meta">
        Ollama: <span data-status={ollamaStatus}>{ollamaStatus}</span>
        {ollamaStatus === 'down' && ' — チャット入力は無効です'}
      </p>
      ...
      <div className="chat-panel">
        ...
        <button type="button" onClick={() => void onSubmit()} disabled={loading || ollamaStatus === 'down'}>
          {loading ? '生成中…' : '送信'}
        </button>
      </div>
        ...
        <div className="chat-result">
          ...
          <ul className="citation-list">
            {result.citations.map((c, i) => (
              <li key={`${c.title}-${i}`}>
                {c.display_id
                  ? <Link to={`/cases/${c.display_id}`}>{c.display_id}</Link>
                  : null}
                {' '}{c.title}
