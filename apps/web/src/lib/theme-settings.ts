...
export const THEME_BOOT_SCRIPT = `(function(){try{var k='aisss-theme-settings';var raw=localStorage.getItem(k);var s=raw?JSON.parse(raw):{mode:'dark',customColors:{}};var m=s.mode||'dark';var eff=m==='system'?(window.matchMedia('(prefers-color-scheme: light)').matches?'light':'dark'):m;document.documentElement.setAttribute('data-theme',eff);var cc=s.customColors||{};Object.keys(cc).forEach(function(key){if(cc[key])document.documentElement.style.setProperty('--'+key,cc[key]);});}catch(e){document.documentElement.setAttribute('data-theme','dark');}})();`
